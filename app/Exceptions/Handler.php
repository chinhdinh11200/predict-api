...
class Handler extends ExceptionHandler
{
    ...
    protected function invalidJson($request, ValidationException $exception): JsonResponse
    {
        if (str_contains($request->getPathInfo(), 'register')) {
            foreach ($exception->validator->failed() as $failedRules) {
                foreach ($failedRules as $failedKey => $value) {
                    if ($failedKey == UserUnique::class) return ResponseHelper::sendResponse($exception->status, trans('response.register_validation.email'), $exception->errors());
                }
            }
        }
